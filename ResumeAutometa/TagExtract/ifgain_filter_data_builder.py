...
class IdListGenerator(object):
    ...
    def get_training_id(self):
        ...
        write_data2json(id_list, IFGAIN_FILTER_EXTRACT_RESOURCES + "id_list.txt")


class IfGainFilterDataBuilder(BatchProc):
    def __init__(self, batch_size, proc_id, chunck_file):
        super(IfGainFilterDataBuilder, self).__init__("train_data_builder")
    ...
    def save2file(self):
        file_name = "_".join(
            ["ifgain_filter_data_training", DB_NAME, DB_TABLE_NAME, str(self.proc_id), str(self.current_chunck_idx)])
        file_name = IFGAIN_FILTER_EXTRACT_RESOURCES + file_name + ".txt"
        ...
        file_name = "_".join(
            ["ifgain_filter_data_testing", DB_NAME, DB_TABLE_NAME, str(self.proc_id), str(self.current_chunck_idx)])
        file_name = IFGAIN_FILTER_EXTRACT_RESOURCES + file_name + ".txt"
        with open(file_name, 'w') as file_handler:
    ...
    def run(self):
        id_list = read_file_json(IFGAIN_FILTER_EXTRACT_RESOURCES + self.chunck_file)
...
def final_step():
    intergrate_files("ifgain_filter_data_testing", IFGAIN_FILTER_EXTRACT_RESOURCES)
    intergrate_files("ifgain_filter_data_training", IFGAIN_FILTER_EXTRACT_RESOURCES)


def run_task(batch_size, proc_id, chunck_file):
    tool = IfGainFilterDataBuilder(batch_size, proc_id, chunck_file)
    tool.main()


def distribute_task(batch_size, task_num=2):
    id_list = read_file_json(IFGAIN_FILTER_EXTRACT_RESOURCES + "id_list.txt")
    id_list.sort()
    ...
    for idx in range(len(proc_chuncks)):
        ...
        write_data2json(proc_chuncks[idx], IFGAIN_FILTER_EXTRACT_RESOURCES + chunck_file)
        time.sleep(1)
