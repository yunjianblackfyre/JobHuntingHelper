...
from ResumeAutometa.Computation.preprocessor import NewWordContentPreproc
from ResumeAutometa.Config.file_paths import NEW_WORDS_EXTRACT_RESOURCES
...
class IdListGenerator(object):
    ...
    def get_training_id(self):
        ...
        write_data2json(id_list, NEW_WORDS_EXTRACT_RESOURCES + "id_list.txt")
...
class TrainDataBuilder(BatchProc):
    def __init__(self, batch_size, proc_id, chunck_file):
        ...
        self.training_data = []
        self.testing_data = []
        self.content_preproc_handler = NewWordContentPreproc()
    ...
    def save2file(self):
        file_name = "_".join(["new_word_training", DB_NAME, DB_TABLE_NAME, str(self.proc_id), str(self.current_chunck_idx)])
        file_name = NEW_WORDS_EXTRACT_RESOURCES + file_name + ".txt"
        ...
        file_name = "_".join(["new_word_testing", DB_NAME, DB_TABLE_NAME, str(self.proc_id), str(self.current_chunck_idx)])
        file_name = NEW_WORDS_EXTRACT_RESOURCES + file_name + ".txt"
        with open(file_name, 'w') as file_handler:
    ...
    def run(self):
        id_list = read_file_json(NEW_WORDS_EXTRACT_RESOURCES + self.chunck_file)
...
def final_step():
    intergrate_files("new_word_testing", NEW_WORDS_EXTRACT_RESOURCES)
    intergrate_files("new_word_training", NEW_WORDS_EXTRACT_RESOURCES)
...
def distribute_task(batch_size, task_num=2):
    id_list = read_file_json(NEW_WORDS_EXTRACT_RESOURCES + "id_list.txt")
    id_list.sort()
    ...
    for idx in range(len(proc_chuncks)):
        ...
        write_data2json(proc_chuncks[idx], NEW_WORDS_EXTRACT_RESOURCES + chunck_file)
        time.sleep(1)
